...
export const userProfileReducer: any = (state = initialState, action: any) => produce(
    state,
    (draft: any) => {
        switch (action.type) {
            case 'PROFILE/GET_USER_INFO':
                return load(draft);
            case 'PROFILE/GET_USER_INFO_SUCCESS':
                return loadSuccess(draft, action.user);
            case 'PROFILE/GET_USER_INFO_ERROR':
